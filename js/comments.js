		...
		commentForm.submit(function(){
			...
			$( '#commentform [aria-required=true]' ).each( function() {
				...
				if ( $.trim( $(this).val() ) == '' ) {
					...
					$(this).parent().append( ' <span class="aac-field-error" id="' + id + '">' + aac.required + '</span>' );
					hasError = true;
				}
			 ...
			 */
			if ( hasError ) {
				statusdiv.html( '<p class="aac-comment-error">' + aac.error + '</p>' ).focus();
				return false;
			}			
			...
			$.ajax({
				...
				success: function( data, textStatus ){
					...
					if ( success ) {
						 ...
						 */						
						statusdiv.html('<p class="aac-comment-success" >'+status+'</p>').focus();
						//alert(data);
						 ...
						 */
						statusdiv.html('<p class="aac-comment-error" >'+status+'</p>').focus();
						commentForm.find('textarea[name=comment]').val('');
					}
				}
			} );
